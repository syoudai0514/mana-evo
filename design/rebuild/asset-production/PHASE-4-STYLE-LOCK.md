...
Status: **APPROVED — GLOBAL RENDERING LANGUAGE**
...
- This approval locks rendering/review rules only. It does **not** approve any individual monster asset as FORMAL.
...
2. Existing CURRENT candidate asset, when present.
...
## 7. Background, crop and baked content

Per-ID production art:
- transparent background preferred; clean white acceptable where required;
- full body visible;
- signature features remain inside the safe crop;
- no text, name, number, badge, UI, scenery, frame or type label baked into the image.
...
### KEEP
Existing CURRENT candidate or compatible historical direction already satisfies identity, family continuity, silhouette distinction, originality, child readability, technical crop/background and small-size readability.

`KEEP` is still only a candidate decision; it is not FORMAL approval.

### REFINE
Useful prior identity exists, but controlled changes are needed for silhouette separation, stage growth, face treatment, detail/VFX reduction, material clarification, crop or originality distance.

### REGENERATE
Required when CURRENT per-ID asset is missing or the prior direction cannot be repaired without retaining a CURRENT conflict, duplicate template, broken family continuity, unreadable small-size design, unsafe age treatment or existing-IP resemblance.

Regeneration must still use compatible 0822 visual cues where useful. It is not permission to invent new lore or change CURRENT identity.
