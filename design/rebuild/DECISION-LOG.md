...
- Decision — Battle XP: D-020のactive/team distribution後、player-vs-enemy level gap multiplierを適用。playerがenemyより`>=15`高い→0.15、`>=10`上→0.25、`>=6`上→0.50、enemyがplayerより`>=3`高い→1.15、`>=5`高い→1.25、その他1.00。old-area farmingを最速育成経路にしない。
...
## D-028 Cloud sync timing / reconciliation / child-safe abnormal visibility
- Status: CONFIRMED_CHANGE / USER-DECISION / REVIEW-REQUIRED
- Baseline / prior CURRENT: D-018 introduced family account + full cloud snapshot and D-021 moved conflict resolution out of child gameplay, but exact autosync timing and the behavior for local/cloud divergence were not fully enumerated in CURRENT. Runtime used roughly 1.4s trailing debounce, so continuous saves could keep postponing cloud delivery; profile switch changed the device-active profile before the explicit flush; overlapping sync calls were not serialized.
- Evidence of approval: 2026-08-29 user explicitly prioritized cloud persistence reliability over optional auth UX work, requested immediate implementation/release only after design documents and review, requested use-case-by-use-case behavior for local/cloud differences in an owner-readable document, and clarified that normal child UI should stay clean while a sync abnormality may surface a minimal warning.
- Decision — local-first timing: learning/game mutations persist locally immediately. Normal cloud autosync uses an approximately **800ms** trailing debounce. The first unsynced save in a continuous burst establishes a **4000ms maximum dirty window** before at least one reconciliation attempt. App/session start, network `online`, focus/foreground return, background/pagehide and profile-switch boundaries request immediate best-effort reconciliation. Network/server failure never invalidates valid local progress; reconnect/resume retries.
- Decision — reconciliation authority: compare current local payload/hash and per-profile parts, last-known synced revision/hash metadata and current cloud revision/payload. Do not choose a winner from device clock/timestamp alone. Outcomes are NOOP, PUSH, PULL, MERGE for disjoint stable-profile changes, or CONFLICT for same-profile divergence/unsafe compatibility. Same-profile conflict never uses silent last-write-wins.
- Decision — profile switch: before active profile A changes to B, cancel pending timers and await reconciliation of the current household/profile state. Only after that boundary may device-local active profile change and reload occur. A and B state must never be copied into each other, and one device must not change another device's active profile selection.
- Decision — in-flight race safety: at most one authoritative reconciliation sequence is in flight per app instance. A later sync request while one is in flight requests a serialized rerun/coalesced reconciliation rather than a competing write. An older response must not falsely mark newer local changes as completely synced.
- Decision — child visibility: healthy cloud operation stays invisible in child gameplay. Debounce wait, in-flight sync, brief offline state and successful retry do not produce routine child cloud UI. A small non-destructive **`保存確認`** warning may appear only for a real attention state: confirmed same-profile conflict or an explicit persistent automatic sync error. It must communicate that current device progress is still locally saved. Cloud-vs-device overwrite/destructive resolution remains Parent-owned and PIN-protected.
- Decision — use-case authority: `design/current/07A-CLOUD-SYNC-USE-CASES.md` is the normative scenario matrix for local-only change, cloud-only change, fresh device, cloud-empty initialization, disjoint-profile merge, same-profile conflict, offline/reconnect, rapid saves, profile switching, in-flight races, TEST isolation and reload/crash recovery. `USER-GUIDE.md` must keep the same cases understandable to a non-technical owner.
- Reason: Safari termination, network loss, multiple family devices and profile switching can occur at arbitrary points. The system should preserve valid progress rather than optimize merely for fewer network calls or newest timestamps. The 800ms/4000ms two-window policy limits cloud lag without blocking child play; explicit reconciliation cases, serialized writes and adult-owned destructive choices reduce data-loss/race risk.
- Affected areas: Save / cloud autosync / profile switching / Parent conflict UX / child save warning / offline recovery / W-107 CURRENT / W-108 acceptance / owner guide. Authentication provider, learning rules, Battle rules, Monster Art and game economy are unchanged.
- Tests required: local dirty event from learning+game / 800ms debounce + 4000ms max dirty / start+online+focus+background+pagehide / local-only push / cloud-only pull / fresh-device adopt / disjoint-profile merge / same-profile conflict / offline→reconnect / current-profile flush before active-profile mutation / serialized in-flight rerun / TEST no-cloud-write / reload-crash local recovery / normal child UI silent / abnormal child `保存確認` only / full canonical sync + unit/integration + build + release-readiness + iPhone WebKit regression.
