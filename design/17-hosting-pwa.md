...
iOSではmanifestのアイコンだけに依存せず、`index.html` で専用 `apple-touch-icon.png` を明示する。

- `apple-mobile-web-app-capable=yes`
- `apple-mobile-web-app-title=マナエボ`
- `apple-touch-icon`: 180x180 PNG

既にホーム画面へ追加済みの端末ではiOS側に旧アイコンがキャッシュされることがある。その場合、アプリ側の不具合ではないため、ホーム画面の旧ショートカットを削除してPages版を再追加する。
...
- 現行cache: `v4`
- install時にapp shell / manifest / 主要アイコンをprecache
- activate時に旧ManaEvo cacheを削除
- navigation: network-first + offline fallback
- 同一originかつ `/mana-evo/` 配下のみ処理
- 静的アセット: cache-first、未取得時network
