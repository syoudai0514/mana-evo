...
## 14. 2026-08-29 Battle V6 production-review hotfix — D-025

D-025 adds authoritative stale/replay safety to the D-022 post-KO opportunity. This does not change capture probability, ball multipliers, max attempts, duplicate choice or post-KO XP=0.

### 14.1 Authoritative battle source

For every post-KO capture eligibility check and attempt, the current persisted `game.activeBattle` is authoritative.

A caller-held/rendered battle object is only a request snapshot. It must not be trusted when it no longer represents the current active battle.

At minimum, a post-KO request must match the authoritative active battle on:

- `battleId`;
- current terminal/capture status needed for the post-KO window;
- the current post-KO opportunity state;
- current capture-attempt state needed to enforce the shared max-3 boundary.

A stale battle from an earlier encounter, or an older snapshot from the same encounter after state has advanced, is rejected rather than being processed against whichever battle happens to be on screen.

### 14.2 Settlement/idempotency before item consumption

Before decrementing the selected ball/item for a post-KO success path, check the canonical capture resolution/settlement identity for that battle.

If that capture settlement already exists, replay must be handled idempotently. It must not:

- consume another ball;
- create another captured instance;
- create another duplicate settlement/choice;
- change XP (post-KO remains `0` additional Battle XP);
- rewrite `activeBattle` from the stale request object.

The same protection applies while a duplicate capture settlement is still pending its keep/support choice: replaying the old post-KO attempt cannot consume a second ball.

### 14.3 Stale rejection is side-effect free

A rejected stale post-KO request must leave authoritative game state unchanged, including:

- `game.activeBattle`;
- capture item counts;
- capture attempt count;
- capture settlement registry;
- BOX/dex/duplicate state;
- Battle XP/reward settlement.

### 14.4 Acceptance addendum

In addition to §13.8 verify:

- old KO battle object cannot capture while another battle is active;
- old KO battle object cannot capture when `activeBattle` is null;
- stale same-battle replay after settlement cannot consume another ball;
- duplicate pending replay cannot consume another ball;
- stale replay never replaces the authoritative active battle;
- valid current post-KO capture still uses the existing max-3 attempt and ball-consumption semantics exactly once.
