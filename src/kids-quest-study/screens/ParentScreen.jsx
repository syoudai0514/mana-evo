...
export default function ParentScreen({ onBack }) {
  ...
  return <div className="screen fade-in parent-screen"><Starfield /><AppHeader onBack={onBack} title="👨‍👩‍👧 おうちのひとへ" right={<span/>}/>
    <div className="scroll-y" style={{flex:1,padding:'4px 8px 28px'}}><div style={{maxWidth:760,margin:'0 auto',display:'flex',flexDirection:'column',gap:18}}>
      <section><h3>🧒 子どもプロフィール</h3><div className="card">
      ...
      <section><h3>📊 とくい・にがて</h3><div style={{display:'grid',gap:8}}>{currentDomains.map((domain)=>{const sk=skillOf(state,domain.id);const today=daily.perDomainToday?.[domain.id];return <div key={domain.id} className="card row" style={{alignItems:'center',gap:12}}><span style={{fontSize:28}}>{domain.emoji}</span><div className="grow"><strong>{domainName(domain,state.grade)}</strong><div className="muted" style={{fontSize:12}}>レベル {Math.floor(sk.level)} ・ きょう {today?`${today.correct}/${today.attempts}`:'0/0'}</div></div><span className="pill">{trendLabel(sk)}</span></div>})}</div><p className="muted" style={{fontSize:12}}>苦手な教科は問題を簡単にするのではなく、ヒントを増やし、復習を適切な間隔で戻します。復習予定は「とっくん」に出ます（現在 {missedCount(state)}こ）。</p></section>

      <section><h3>🎓 学年・先取り</h3><div className="card"><p className="muted" style={{lineHeight:1.6}}>現在: <b>{gradeOf(state.grade).short}</b> ／ 解放済み: <b>{gradeOf(state.gradeMax).short}</b> まで。通常は必須単元を別日に定着させ、ほしのしれんをクリアすると次が開きます。保護者判断では先取り解放できます。</p>
        <div className="row wrap" style={{gap:8}}>{state.gradeMax<MAX_GRADE && <button className="btn btn--sun" onClick={()=>dispatch({type:'FORCE_GRADE_MAX',gradeMax:state.gradeMax+1})}>⏭ {gradeOf(state.gradeMax+1).short} を先取り解放</button>}{state.gradeMax>0 && <button className="btn btn--ghost" onClick={()=>dispatch({type:'LOWER_GRADE_MAX',gradeMax:state.gradeMax-1})}>⏪ {gradeOf(state.gradeMax-1).short} までに戻す</button>}</div>
        <div className="row wrap" style={{gap:6,marginTop:10}}>{GRADES.filter(g=>g.id<=state.gradeMax).map(g=><button key={g.id} className={'btn '+(g.id===state.grade?'btn--primary':'btn--ghost')} onClick={()=>dispatch({type:'SET_GRADE',grade:g.id})}>{g.short}</button>)}</div>
      </div></section>

      <section><h3>🚫 子どもが選べる学年の下限</h3><div className="card"><p className="muted" style={{fontSize:13,lineHeight:1.6}}>解放済み学年は子どもが自由に行き来できます。簡単な学年へ戻りすぎないよう、必要なら下限を設定できます。</p><div className="row wrap" style={{gap:6}}><button className={'btn '+((state.settings.minSelectableGrade||0)===0?'btn--primary':'btn--ghost')} onClick={()=>dispatch({type:'SET_MIN_SELECTABLE_GRADE',grade:0})}>せいげんなし</button>{GRADES.filter(g=>g.id>0&&g.id<=state.gradeMax).map(g=><button key={g.id} className={'btn '+(g.id===state.settings.minSelectableGrade?'btn--primary':'btn--ghost')} onClick={()=>dispatch({type:'SET_MIN_SELECTABLE_GRADE',grade:g.id})}>{g.short}〜</button>)}</div></div></section>

      <section><h3>🔥 むずかしいモード</h3><div className="card"><p className="muted" style={{fontSize:13,lineHeight:1.6}}>通常教材とは習熟度・復習記録を分けて保存します。小1から算数・国語の先取り/パズル、小4以降は理科・社会・英語の発展問題も使います。</p><div className="row" style={{gap:8}}><button className={'btn '+(state.settings.mode==='normal'?'btn--primary':'btn--ghost')} onClick={()=>setSetting('mode','normal')}>ふつう</button><button className={'btn '+(state.settings.mode==='hard'?'btn--pink':'btn--ghost')} onClick={()=>setSetting('mode','hard')}>🔥 むずかしい</button></div></div></section>

      <section><h3>🗣️ 読み上げ・つくよみちゃん</h3><div className="card" style={{display:'grid',gap:12}}>
      ...
      <section><h3>📦 学習データひきつぎ</h3><div className="card" style={{display:'grid',gap:8}}><button className="btn btn--ghost" onClick={exportData}>📋 学習データをコピー</button><textarea value={importText} onChange={e=>setImportText(e.target.value)} placeholder="バックアップコードを貼り付け" style={{minHeight:90,borderRadius:12,padding:10}}/><button className="btn btn--ghost" disabled={!importText.trim()} onClick={importData}>⬇️ 読み込む</button>{message && <div className="muted">{message}</div>}</div></section>
    </div></div>
  </div>
}
